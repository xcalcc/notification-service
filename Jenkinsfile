pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE_TAG_REMOTE = "127.0.0.1:8129/xcalibyte/xcal.notification-service";
    }
    stages {
        stage('pull') {
            steps {
                echo '========= pull =========='
                git credentialsId: 'github_user', branch: 'master', url: 'https://github.com/xcalcc/notification-service.git'
            }
        }
        stage('build') {
            steps {
                ...
                script {
                    ...
                    //sh 'cnpm i'
                    //sh 'npm run build'
                    //sh 'cd /home/xc5/dev_deploy/notification-service'
                    sh 'cp .env.default .env'
                    withCredentials([usernamePassword(credentialsId: 'harbor_user', passwordVariable: 'password', usernameVariable:'user')]) {
                        sh """
                    ...
                    sshPublisher(
                      ...
                      publishers: [
                        sshPublisherDesc(
                          ...
                          transfers: [
                            sshTransfer(
                              execCommand: "cd /home/xc5/dev_deploy/notification-service && docker pull $DOCKER_IMAGE_TAG_REMOTE:latest && sh restart.sh"
                            )
                          ])
                      ])
                }
